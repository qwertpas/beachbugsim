    ...
    public static class MotionProfile {
        ...
        public MotionProfile(double vmax_input, double amax_input, double amin_input, double target_input) {
            ...
            if (time1 < time2) {
                ...
                times = new double[] { 0.0, time1, time2, time3 }; // added a zero in the first index so times[1] is
                                                                   // time1
            } else {
                isTrapezoid = false;
        ...
        public MotionProfilePoint getPoint(double time) {
            ...
            if (isTrapezoid) {
                ...
                if (time < times[1]) {
                    ...
                    dist = (0.5 * amax * times[1] * times[1]) + (0.5 * (time - times[1])
                            * (amax * times[1] + amax * times[1] - (amin * (time - times[1]))));
                } else if (time > times[2]) {
                    done = true;
                    accel = 0;
                    velo = 0;
